...
2  -  3		A2		Numeric State Code.   Range is 01-62.  Data records are in order by ORI code within numeric state code. The values are:
...
11 - 12		A2		Group. Group 0 is possessions;  1 - 7 are cities; 8 - 9 are counties.  Sub-group (position 12) is blank when not used. All populations are inclusive.  Values are:
...
119			A1		Special Mailing Group.   When addressing RETURN-A's,  the tape is sorted by Zip Code so that the forms can be mailed by geographic area.  Special Mailing Group agencies are excluded
